...
Build AI-powered climbing coach that analyzes logged data and provides actionable training guidance through weekly dashboard recommendations and free-form chat. The journey builds from database foundations and services (Phase 18) through UI with mock data (Phase 19) to real LLM integration (Phase 20) and culminates in an interactive chat interface (Phase 21). Later phases refined the coaching experience with projecting focus recommendations, user context, usage limits, enhanced prompts, and markdown rendering.
...
**Details:**
Created three database tables (coach_recommendations, coach_messages, coach_api_usage) with RLS policies, JSONB content fields, and performance indexes. Built patterns.ts service for extracting failure patterns, style weaknesses, and climbing frequency. Implemented coach.ts service with LLM API abstraction, cost tracking, and rate limiting. Created TanStack Query hooks (useCoach, useCoachMessages) for state management. Added privacy safeguards to anonymize data before AI processing.
...
**Details:**
Built complete coach page UI with Radix UI tabs for Recommendations and Pattern Analysis. Created RecommendationsDisplay component showing weekly focus and 3 drills. Added loading states, error handling, and offline caching support via TanStack Query. Provided clear entry points to chat from recommendations page.
...
**Details:**
Created openrouter-coach Edge Function with JWT authentication and OpenRouter API integration using OpenAI SDK. Implemented climbing-specific prompt engineering with JSON output format. Added validation, retry logic, and graceful fallback on API failures. Integrated privacy validation before LLM calls.
...
**Details:**
Implemented Server-Sent Events (SSE) streaming in openrouter-chat Edge Function with JWT authentication. Created client-side SSE service using @microsoft/fetch-event-source. Built React chat interface with message bubbles, typing indicator, and mobile-optimized input. Added route configuration, loading states, error handling, and visual polish.
...
**Details:**
Added OPENROUTER_MODEL environment variable configuration for both chat and coach Edge Functions. Replaced custom calculateCost() functions with OpenRouter's provided usage.cost field for accurate cost tracking. Documented configuration in .env.example.
...
**Details:**
Extended AnonymizedClimb type with notes and date fields. Refactored system prompt to technique-first coaching philosophy (strength failures = technique gaps). Added extractRecentClimbs function to fetch last 30 climbs for LLM context. Updated Edge Function and client service to include recent climbs. Added measurable_outcome field validation to drill schema. Updated example output to technique-focused drill.
...
**Details:**
Extended Edge Function system prompt to generate 3-4 projecting focus areas with qualitative grade guidance (e.g., "slightly above max grade") and gym limitation awareness. Updated TypeScript types to include ProjectingFocus interface. Added Projecting Focus section to coach-page UI below drills using existing FormSection pattern.
...
**Details:**
Added climbing_context TEXT column to profiles table with 2000 character limit via database check constraint. Created settings UI with textarea for users to describe their climbing style, goals, and preferences. Integrated climbing context into both coach recommendations and chat system prompts. Followed ChatGPT's "custom instructions" pattern.
...
**Details:**
Updated README.md to document the new AI Coach features including coach recommendations, pattern analysis, chat interface, projecting focus, and climbing context configuration. Included screenshots (TODO), setup instructions, privacy safeguards, and usage examples.
...
**Details:**
Created user_limits table with rec_count and chat_count columns and atomic RPC increment functions with UTC midnight reset. Updated both Edge Functions to check limits BEFORE LLM API calls to control costs. Created useUserLimits hook with staleTime: 0 for fresh data. Added inline usage counters ("X/2 used today", "X/10 used today") next to action buttons. Disabled buttons at limit with inline error showing time until reset.
...
**Details:**
Reworked the chat system prompt to improve quality and relevance of responses. Updated data passed to chatbot to include the latest generated recommendation so users can discuss and ask questions about their current weekly focus, drills, and projecting focus areas. Established clear coaching persona with reactive behavior (only reference recommendations when user asks), concept-first explanations (teach technique, then mention drill names), and graceful missing data handling.
...
**Details:**
Added markdown rendering to chat bubbles using react-markdown (industry standard, secure by default), remark-gfm (for tables/tasklists), and rehype-highlight (code syntax highlighting). Only renders markdown for assistant messages; user messages remain plain text to meet "what you type is what you see" expectations. Dark-themed components match app's color system (#09090b background, white/gray text).

## Milestone Summary

**Decimal Phases:**
None - all phases in milestone are sequential integers (18-29).

**Key Decisions:**
- Used OpenRouter API with OpenAI SDK for flexible model selection
- Edge Functions for server-side LLM calls with JWT authentication
- SSE streaming for real-time chat responses (simpler than WebSockets)
- JSONB for recommendation content to support schema evolution
- Atomic daily limit counters via PostgreSQL RPC functions
- Technique-first coaching philosophy (strength failures = technique gaps)
- Daily limits: 2 recommendations/day, 10 chat messages/day to control costs
- Privacy-first: anonymize data before LLM processing, RLS for user isolation
- Graceful degradation: all features work with missing data or API failures

**Issues Resolved:**
- LLM API cost control: implemented daily usage limits with atomic counters
- Generic advice: added pattern analysis and recent climb context to prompts
- Limited personalization: added user climbing context field
- Chat without recommendations: graceful degradation handles missing data
- Cost tracking accuracy: switched to OpenRouter's provided usage.cost field

**Issues Deferred:**
- None - all planned features delivered

**Technical Debt Incurred:**
- Screenshots need capture: README includes TODO comments for coach page and chat page screenshots
- Edge Function deployment required: users must deploy updated Edge Functions after this milestone
