...
must_haves:
  ...
  key_links:
    ...
    - from: "src/lib/coachUtils.ts"
      ...
      via: "Uses AnonymizedClimb, Style, FailureReason types for type safety"
      pattern: "import type { AnonymizedClimb, Style, FailureReason } from '@/types'"
---
...
export interface AnonymizedClimb {
  ...
  grade_value: string
  climb_type: string
  style: Style[]  // Type alias from existing types
  outcome: string
  awkwardness: number
  failure_reasons: FailureReason[]  // Type alias from existing types
}
```

Add after PatternAnalysis interfaces, before existing Climb interface.

Note: Style and FailureReason are type aliases already defined in types/index.ts:
- Style: union type of string values like 'Slab', 'Vert', 'Overhang', etc.
- FailureReason: union type combining PhysicalReason, TechnicalReason, MentalReason

Why: This type defines the shape of data safe to send to external LLM APIs, excluding PII.
  </action>
  <verify>Run `pnpm typecheck` - no errors. Verify type exists: `grep -c "AnonymizedClimb" src/types/index.ts` shows at least 1 match. Verify Style and FailureReason are type aliases: `grep "export type Style\|export type FailureReason" src/types/index.ts` shows they are defined as type aliases.</verify>
...
```typescript
import type { Climb, Style, FailureReason, AnonymizedClimb } from '@/types'
...
export function anonymizeClimbsForAI(climbs: Climb[]): AnonymizedClimb[] {
  return climbs.map((climb) => ({
    ...
    style: climb.style,  // Style[] from type alias
    outcome: climb.outcome,
    awkwardness: climb.awkwardness,
    failure_reasons: climb.failure_reasons,  // FailureReason[] from type alias
  }))
}
...
- Uses existing type aliases Style[] and FailureReason[] from types/index.ts
...
- Style and FailureReason imported correctly as type aliases from @/types
</verification>
...
- Type aliases Style and FailureReason correctly used
