...
must_haves:
  truths:
    ...
    - API call duration is measured and logged with warnings if exceeds 3000ms (EXTR-07)
  artifacts:
...
<objective>
Edge Function implementation for AI tag extraction - validates JWT, enforces quota, calls OpenRouter API with structured JSON, validates response, updates climb with tags, tracks costs, and measures API call duration with performance logging. Returns immediately (non-blocking) to avoid blocking save flow.

Purpose: Core backend service for EXTR-01, EXTR-02, EXTR-05, EXTR-06, EXTR-07 (3-second performance). Never blocks climb save - runs async after persist.
...
<task type="auto">
  ...
  <action>
    ...
    7. OpenRouter API call with duration measurement (EXTR-07 - 3-second performance):
    ...
    8. Duration measurement and logging (EXTR-07):
       - Before OpenRouter call: const startTime = performance.now()
       - After OpenRouter response: const duration = performance.now() - startTime
       - Log "Tag extraction API call duration: {duration.toFixed(0)}ms"
       - If duration > 3000ms: Log warning "Tag extraction exceeded 3s target: {duration.toFixed(0)}ms"
       - Include climb_id in logs for debugging

    9. Response validation (from RESEARCH.md lines 487-537):
    ...
    10. Retry logic (from RESEARCH.md lines 214-263):
        - MAX_RETRIES = 2
    ...
    12. Track API usage (from RESEARCH.md lines 386-405):
  ...
  <verify>
    ...
    Test: Verify API call duration is logged by checking Supabase function logs. Use test notes (100-500 characters) and confirm duration appears in logs with warning if exceeds 3000ms.
  </verify>
  <done>Edge Function validates JWT, enforces quota, extracts tags with validation, updates climb, tracks costs, measures API duration with performance logging, returns immediately</done>
</task>
...
<success_criteria>
openrouter-tag-extract Edge Function deployed and functioning. Enforces 50 climbs/day quota, validates tags (confidence >= 70, max 3 styles, 1-3 failure reasons), tracks costs, measures API call duration with performance logging, and never blocks save flow.
</success_criteria>
