...
must_haves:
  truths:
    - "Edge Function generates 3-4 projecting focus areas with all required fields"
    - "Malformed projecting_focus responses are rejected with clear error messages"
    - "Projecting focus recommendations include qualitative grade guidance"
    - "Recommended focus areas use styles commonly found in gyms (e.g., crimpy overhangs) and avoid rare styles (e.g., dyno toe hooks)"
  artifacts:
